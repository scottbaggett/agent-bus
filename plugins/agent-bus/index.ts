...
function runner(directory: string) {
  ...
  return {
    ...
    /** Env stamped onto shell children so a seat's shell and hooks agree. */
    shellEnv(sessionID = executing) {
      return env(sessionID && sessions.has(sessionID) ? sessionID : undefined)
    },
    digest: (sessionID: string) => run(["digest"], sessionID),
    /**
     * Ask the CLI whether this seat should be continued. Returns the wake text
     * or "". stop-hook is the single source of truth for watch on/off, unread
     * selection, MAX_SHOWS, WAKE_BUDGET, and the PM stale-thread nag.
     */
    async wakeText(sessionID: string) {
  ...
  async setup(ctx: Plugin.Context) {
    ...
    const inject = async (sessionID: string, text: string, wake: boolean) => {
      try {
        ...
        // Delivery is best-effort; the packet itself never lives in the wake
        // channel, so a dropped injection only delays it to the next digest.
      }
    }
          ...
          try {
            ...
            switch (event.type) {
              ...
              case "session.idle": {
                ...
                const text = await bus.wakeText(sid)
                if (text) await inject(sid, text, true)
                break
              }
            }
          } catch {
    ...
    const stopPolling = pollEvery(() => {
      for (const sessionID of bus.sessions.keys()) {
        void (async () => {
          const text = await bus.wakeText(sessionID)
          if (text) await inject(sessionID, text, true)
        })()
      }
    })
